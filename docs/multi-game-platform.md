# Multi-game platform — design notes

A speculative design for evolving the current Brass-Birmingham host into
a **platform** that hosts multiple board games behind a shared
account/lobby/save system. Brass becomes one game among many, called
**Coke and Iron** at the user-facing level. New games plug in via a
single interface.

This document captures the architecture, the trade-offs, the database
shape, and a phased rollout. It is **not** a commitment — it's the
written-up version of a design conversation, written so a future session
can pick it up cold.
