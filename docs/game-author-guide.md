...
A spec for adding a new game to the platform. Hand this document (plus
`docs/multi-game-platform.md`) to whoever is implementing the next game
— it's everything they need to interface with the platform cleanly,
without reading platform internals.

The platform is **game-agnostic**: it owns auth, tables, slots, saves,
